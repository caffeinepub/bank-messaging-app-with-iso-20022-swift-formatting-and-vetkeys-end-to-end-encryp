 ...
 */
export async function exportTransportKeyPair(keyPair: TransportKeyPair): Promise<ExportedTransportKeyPair> {
 ...
 */
export async function importTransportKeyPair(exported: ExportedTransportKeyPair): Promise<TransportKeyPair> {
  // Decode base64 to ArrayBuffer
  const publicKeyBuffer = Uint8Array.from(atob(exported.publicKey), c => c.charCodeAt(0)).buffer;
  const privateKeyBuffer = Uint8Array.from(atob(exported.privateKey), c => c.charCodeAt(0)).buffer;
